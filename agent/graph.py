...
def _parse_verdict(text: str) -> dict:
    """Parse a {"ok": bool, "issue": str} verdict out of an LLM reply.

    The model may wrap the JSON in prose or a ```json fence, so we grab the
    first balanced-looking object and json.loads it. On any failure we default
    to ok=True (don't loop forever on an unparseable verifier reply) but surface
    the raw text as the issue so it's visible in history/traces.
    """
